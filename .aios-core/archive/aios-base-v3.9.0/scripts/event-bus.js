 ...
 * Example: clinical detects emergency → patient-ops reacts → finance creates billing
 * 
 * Usage:
 *   const bus = require('./event-bus');
 *   bus.on('patient.emergency', (data) => handleEmergency(data));
 *   bus.emit('patient.emergency', { severity: 'high', patientId: '123' });
 * 
...
class AIOSEventBus extends EventEmitter {
     ...
     * @param {string} channel - Event channel (e.g., 'patient.emergency', 'finance.invoice')
     * @param {string} subscriber - Subscriber ID (e.g., '@patient-ops-head')
    ...
    publish(channel, data, meta = {}) {
        ...
        if (parts.length > 1) {
            this.emit(`${parts[0]}.*`, event); // e.g., 'patient.*'
        }
...
const CHANNELS = {
    // Patient Operations
    'patient.intake': 'New patient intake started',
    'patient.scheduled': 'Appointment scheduled',
    'patient.emergency': 'Emergency detected',
    'patient.feedback': 'Patient sent feedback',
    'patient.churn_risk': 'Churn risk detected',

    // Clinical
    'clinical.protocol_alert': 'Protocol violation detected',
    'clinical.quality_issue': 'Quality metric below threshold',
...
if (process.argv.includes('--demo')) {
    ...
    bus.subscribe('patient.emergency', '@patient-ops-head', (event) => {
        console.log(`  🏥 @patient-ops-head received: ${JSON.stringify(event.data)}`);
    });

    bus.subscribe('patient.emergency', '@clinical-head', (event) => {
        console.log(`  ⚕️ @clinical-head received: ${JSON.stringify(event.data)}`);
    });

    bus.subscribe('patient.*', '@analytics-head', (event) => {
    ...
    // Simulate events
    console.log('📡 Emitting patient.emergency...\n');
    bus.publish('patient.emergency', {
        severity: 'high',
        patientId: 'P-2024-001',
        description: 'Allergic reaction during procedure',
    }, {
        source: '@clinical-protocol-manager',
        priority: 'critical',
    });
