...
commands:
  ...
  # DooMMasteRBot V5 Fusion — Revenue Operations & Defense (v5.0.0)
  - name: revenue-check
    args: '{operação}'
    description: 'Revenue Gate — Auto-P&L + Juiz de Caixa + Profit Firewall check'
  - name: critic-loop
    args: '{output}'
    description: 'Critic-Gate universal — Pipeline Planner→Solver→Crítico com 7 rubricas (score ≥8)'
  - name: profit-firewall
    args: '{campanha}'
    description: 'Profit Firewall check — brownout se margem < threshold configurado'
  - name: arena
    args: '{variantes...}'
    description: 'Arena Tournament — Thompson Sampling bayesiano entre variantes (mín. 3)'
  - name: security-scan
    args: '{escopo}'
    description: 'Antifragile Defense — OPA gates + Privacy Budget + C2PA check'
  - name: revenue-report
    args: '{período}'
    description: 'Revenue Intelligence — CCR++ causal + Auto-P&L + Replay Lab D+1'
...
dependencies:
  tasks:
    ...
    - ids-governor.md
    # V5 Fusion — Core Revenue/Defense Tasks
    - critic-gate.md
    - revenue-gate.md
    - arena-tournament.md
  ...
  data:
    ...
    # V5 Fusion — Core Frameworks
    - revenue-operations-framework.md
    - antifragile-defense-framework.md
  ...
  checklists:
    ...
    # V5 Fusion — Revenue Checklist
    - revenue-launch-checklist.md
...
**V5 Fusion — Revenue & Defense:**

- `*revenue-check {op}` - Revenue Gate (Auto-P&L + Juiz de Caixa)
- `*critic-loop {output}` - Critic-Gate (Planner→Solver→Crítico)
- `*profit-firewall {camp}` - Profit Firewall check
- `*arena {variantes}` - Arena Tournament (Thompson Sampling)
- `*security-scan {escopo}` - Antifragile Defense scan
- `*revenue-report {período}` - Revenue Intelligence report
...
- Revenue ops → Use `@doom-master` for full Doombot squad
