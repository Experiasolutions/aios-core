...
> **Version:** 2.0.0 | **Ratified:** 2025-01-30 | **Last Amended:** 2026-02-13 (DooMMasteRBot V5 Fusion)
...
### VII. Revenue-First (MUST)

> *DooMMasteRBot V5 Fusion — 2026-02-13*

Toda saída de alto impacto DEVE ser avaliada por métricas financeiras, não por eloquência.

**Regras:**
- MUST: Saídas de oferta, copy, e campanha avaliadas por `P(aceite) × margem × payback`
- MUST: KPIs canônicos (T1C, Taxa de Aceite, Margem) governam decisões de priorização
- MUST: Profit Firewall ativado antes de qualquer lançamento (brownout se margem < threshold)
- SHOULD: Arena de Ofertas/Headlines para testar variantes antes de commit
- SHOULD: Auto-P&L por operação para visibilidade em tempo real

**Gate:** `run-quality-gate.md` (Doombot squad) + `revenue-gate.md` (core)

---

### VIII. Antifragile Defense (MUST)

> *DooMMasteRBot V5 Fusion — 2026-02-13*

Segurança, privacidade e compliance são by-default — nunca bolted-on.

**Regras:**
- MUST: OPA gates (Policy-as-Code) verificados em saídas de agents
- MUST: Privacy Budget por operação (estourou cota = fallback dados sintéticos)
- MUST: C2PA manifest em saídas criativas publicadas
- SHOULD: Red Team Self-Play em operações críticas
- SHOULD: SBOM/SLSA para componentes de software

**Gate:** `security-gate-checklist.md` (Doombot squad)
...
Toda resposta de alta aposta passa pelo pipeline Planner→Solver→Crítico.

**Regras:**
- MUST: Outputs de alta aposta (ofertas, copy final, scripts de venda) passam pelo Critic-Gate
- MUST: 7 rubricas avaliadas (clareza, promessa, prova, preço, prazo, CTA, objeções) — score ≥ 8
- MUST: Se score < 8 em qualquer rubrica → rewrite automático (max 3 loops)
- MUST: Após 3 loops sem atingir SLO → escalate para humano
- SHOULD: Juiz de Caixa como tie-breaker entre alternativas

**Gate:** `critic-gate.md` (core) + `run-quality-gate.md` (Doombot squad)
