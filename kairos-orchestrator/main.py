...
# Force UTF-8 on Windows console to handle emoji in logs
...
from workers.night_shift_worker import fetch_pending_tasks, claim_task, process_task, get_supabase as ns_get_supabase
...
async def main() -> None:
    ...
    # Processar tasks (legado task_queue) — a cada 30 minutos
    scheduler.add_job(
        job_process_tasks,
    ...
    # 🌙 SKYDRA Night Shift — consome fila kairos_task_claims a cada 30s
    async def job_night_shift() -> None:
        """Processa tasks enfileiradas pelo Antigravity via hydra_queue_task."""
        try:
            db = ns_get_supabase()
            tasks = fetch_pending_tasks(db, limit=3)
            for task in tasks:
                if claim_task(db, task["id"]):
                    process_task(db, task)
        except Exception as e:
            logger.error("Night Shift loop error: %s", e)
